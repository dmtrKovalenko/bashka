...
pub fn run(shell: &str, args: Vec<String>) -> Result<i32> {
    ...
    let approval = if vetted.contains(&hash) {
        ...
        session.prompter.say(&line);
        match session.vet(source.clone(), label.clone(), depth)? {
            Some(a) => a,
            None => return Ok(1),
        }
    };
...
mod tests {
    ...
    fn parse(args: &[&str]) -> (String, Vec<String>) {
        let (input, rest) =
            parse_shell_args(&args.iter().map(|s| s.to_string()).collect::<Vec<_>>());
