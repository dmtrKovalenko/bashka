...
impl Prompter {
    ...
    fn on_tty(&mut self, bin: &str, args: &[String]) -> bool {
        let (Ok(tin), Ok(tout)) = (
    ...
    pub fn decide(&mut self, pre: Decision, question: &str, default: bool) -> Result<bool> {
        match (pre, self.auto) {
            (Decision::Yes, _) => Ok(true),
            (Decision::No, _) => Ok(false),
            (Decision::Ask, Auto::Yes) => Ok(true),
            (Decision::Ask, Auto::No) => Ok(false),
    ...
    pub fn spinner(&mut self, message: String) -> Spinner {
        ...
        let spinner = Spinner::start(target, message.clone());
