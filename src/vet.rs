...
impl Session {
    ...
    pub fn vet(&mut self, source: String, label: String, depth: usize) -> Result<Option<Approval>> {
        let (outcome, analysis) = self.analyze(source.clone(), label, depth)?;
    ...
    fn analyze_with_agent(&mut self, summary: &str, source: &str) -> Result<()> {
        ...
        let mut options: Vec<(char, String)> = available
            .iter()
            .enumerate()
            .map(|(i, a)| (char::from(b'1' + i as u8), a.name.to_string()))
            .collect();
        ...
        if choice == 'c' {
            match agents::copy_to_clipboard(&prompt) {
                Some(tool) => self.prompter.say(&format!(
