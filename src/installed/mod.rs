...
pub fn run_tracked(
        ...
        None => match name::derive(
            ...
            None => {
                ...
                for b in &changes.binaries {
                    msg.push_str(&format!("   {} {}\n", paint(ui::DIM, "+"), tilde(b)));
                }
                for d in &changes.created_dirs {
                    msg.push_str(&format!("   {} {}/\n", paint(ui::DIM, "+"), tilde(d)));
                }
    ...
    pkg.version = pkg
        ...
        .unwrap_or_else(|| store::UNKNOWN_VERSION.into());
    let mut lock = store::load()?;
...
fn describe_record(pkg: &Package, changes: &track::Changes) -> String {
    ...
    out.push('\n');
    for b in &changes.binaries {
        out.push_str(&format!("   {} {}\n", paint(ui::GREEN, "+"), tilde(b)));
    }
    for d in &changes.created_dirs {
        out.push_str(&format!("   {} {}/\n", paint(ui::GREEN, "+"), tilde(d)));
    }
    out.push_str(&paint(
        ui::DIM,
...
pub fn list(long: bool) -> Result<String> {
    ...
    out.push_str(&paint(
        ui::DIM,
        format!(
            "\n{} package(s) in {}  ·  `bashka info <name>` for details\n",
            lock.packages.len(),
            tilde(&store::path()?)
        ),
    ));
...
pub fn update(name_arg: &str, current: &Options) -> Result<i32> {
    ...
    if current.config.is_some() {
        opts.config = current.config.clone();
    }
    if opts.yes {
    ...
    let Some(approval) = session.vet(source.clone(), url.to_string(), 0)? else {
        return Ok(1);
    };
    let inherit = Inherit {
        depth: 0,
